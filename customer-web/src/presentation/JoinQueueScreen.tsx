...
export function JoinQueueScreen({ customer, queueUseCases, staffRepo, serviceRepo, settingsRepo, ticketRepo, onTicketCreated }: Props) {
  ...
  const handleJoin = async (event: React.FormEvent) => {
    ...
    if (!phoneNumber.trim()) {
      setError('رقم الهاتف مطلوب لإكمال الحجز.');
      return;
    }
    ...
    setLoading(true);
    try {
      const savedPhone = phoneNumber.trim();
      const { error: phoneError } = await supabase.from('customers').update({ phone_number: savedPhone }).eq('id', customer.id);
      if (phoneError) throw phoneError;
      const ticket = await queueUseCases.joinQueue(customer.id, selServiceId, targetHeroId, savedPhone);
  ...
  if (!isAccepting) return <div className="mobile-screen"><EmptyState title="الحجز الإلكتروني مغلق حالياً" description="الصالون ممتلئ الآن ولا يستقبل حجوزات جديدة عن بُعد. يمكنك زيارتنا حضورياً أو المحاولة لاحقاً." /></div>;

  return (
    <div>
      <div className="mobile-context"><div><div className="mobile-kicker">مرحباً بعودتك</div><div className="mobile-context-name">{customer.name}</div></div><StatusBadge tone={limitReached ? 'warning' : 'neutral'}>{reservationsUsed}/2 حجوزات اليوم</StatusBadge></div>
      <div className="mobile-screen-header"><div className="eyebrow">حجز دور جديد</div><h1 className="mobile-screen-title">احجز مكانك في الدور</h1><p className="mobile-screen-copy">اختر الخدمة والحلاق، وسنخبرك بالوقت المتوقع قبل أن تؤكد.</p></div>
      <div className="mobile-progress" aria-label="تقدم الحجز"><span style={{ width: `${selServiceId ? (selStaffId ? '100%' : '66%') : '33%'}` }} /></div>
      ...
      {limitReached ? <Panel><EmptyState title="اكتملت حجوزاتك اليوم" description="يمكنك استخدام الحجز الإلكتروني مرتين يومياً. عد غداً لحجز دور جديد." /></Panel> : (
        <form className="form-stack" onSubmit={handleJoin}>
          ...
          {selServiceId && selStaffId && <Panel><div className="step"><div className="step-heading"><span className="step-number">03</span><h2 className="step-title">رقم للتواصل وإعادة فتح التذكرة</h2></div><div className="form-group"><label className="form-label" htmlFor="customer-phone">رقم الهاتف <span className="field-hint">(مطلوب)</span></label><div style={{ display: 'flex', gap: 8, alignItems: 'center' }}><span className="status-badge status-neutral" dir="ltr">+20</span><input id="customer-phone" className="text-input" type="tel" inputMode="numeric" autoComplete="tel" value={phoneNumber} onChange={(event) => setPhoneNumber(event.target.value)} placeholder="01012345678" required dir="ltr" aria-describedby="phone-hint" /></div><div className="form-hint" id="phone-hint">سنحفظه في حسابك لاستخدامه لاحقاً، ويجب إدخاله لإكمال الحجز.</div></div></div></Panel>}
          {selServiceId && selStaffId && <Panel><div className="ui-panel-header"><div><h2 className="ui-panel-title">راجع الحجز</h2><p className="ui-panel-note">تأكد من التفاصيل قبل دخول الدور.</p></div><Icon name="check" size={22} color="#7a452d" /></div><div className="summary-list"><div className="summary-row"><span className="summary-label">الخدمة</span><span className="summary-value">{selectedServiceName}</span></div><div className="summary-row"><span className="summary-label">الاختيار</span><span className="summary-value">{selectedHeroName}</span></div><div className="summary-row"><span className="summary-label">الحجوزات المتبقية اليوم</span><span className="summary-value">{2 - reservationsUsed}</span></div></div><div className="mobile-primary-action"><Button type="submit" className="ui-button-wide" disabled={loading}>{loading ? 'جارٍ تأكيد دورك…' : 'تأكيد ودخول الدور'}</Button></div></Panel>}
        </form>
      )}
    </div>
  );
