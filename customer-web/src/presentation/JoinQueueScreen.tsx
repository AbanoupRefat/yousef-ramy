    ...
    <div>
      ...
      {limitReached ? <Panel><EmptyState title="اكتملت حجوزاتك اليوم" description="يمكنك استخدام الحجز الإلكتروني مرتين يومياً. عد غداً لحجز دور جديد." /></Panel> : (
        <form className="form-stack" onSubmit={handleJoin}>
          ...
          {selServiceId && selStaffId && <Panel><div className="step"><div className="step-heading"><span className="step-number">03</span><h2 className="step-title">رقم للتواصل وإعادة فتح التذكرة</h2></div><div className="form-group"><label className="form-label" htmlFor="customer-phone">رقم الهاتف</label><div style={{ display: 'flex', gap: 8, alignItems: 'center' }}><span className="status-badge status-neutral" dir="ltr">+20</span><input id="customer-phone" className="text-input" type="password" inputMode="numeric" autoComplete="tel" value={phoneNumber} onChange={(event) => setPhoneNumber(event.target.value)} placeholder="•••••••••••" dir="ltr" aria-describedby="phone-hint" /></div><div className="form-hint" id="phone-hint">سنحفظه في حسابك لاستخدامه لاحقاً، ولن يظهر رقمه على شاشة الدور.</div></div></div></Panel>}
